...
// Detects a fragmented MP4 (moof/sidx boxes). YouTube/DASH sources yield these:
// the moov holds no sample table and frames live in fragments. Clone Hero's video
// player only decodes a flat MP4 (single moov with sample tables + mdat), so a
// fragmented file plays as a black screen even when the codec is H.264. Scans only
// top-level box headers (seeks past payloads), so it is cheap.
function isFragmentedMp4(filePath) {
  ...
  try {
    ...
    while (pos + 8 <= size) {
      ...
      if (type === 'moof' || type === 'sidx') return true;
      if (boxSize === 1) boxSize = Number(hdr.readBigUInt64BE(8)); // 64-bit size
      if (boxSize < 8) break;
      pos += boxSize;
    }
  } catch {
    // Unreadable — treat as non-fragmented and let downstream handle it.
  } finally {
...
function ensureCloneHeroVideo(videoPath) {
  ...
  if (codec === 'h264') {
    // Codec is fine, but a fragmented container still won't play — flatten it
    // (stream copy, no re-encode) into a standard moov+mdat mp4 with faststart.
    if (!isFragmentedMp4(videoPath)) return true;
    console.log('  ↻ Flattening fragmented MP4 container for Clone Hero...');
    const flatPath = videoPath.replace(/\.mp4$/, '.flat.mp4');
    const res = spawnSync('ffmpeg', [
      '-y', '-i', videoPath, '-c', 'copy', '-movflags', '+faststart', flatPath,
    ], { stdio: 'ignore', timeout: 120000 });
    if (res.status === 0 && fs.existsSync(flatPath)) {
      fs.renameSync(flatPath, videoPath);
    } else {
      try { fs.unlinkSync(flatPath); } catch {}
      console.log('  ⚠ Could not flatten video container; leaving original.');
    }
    return true;
  }
